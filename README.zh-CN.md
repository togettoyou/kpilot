...
- **每集群一条反向连接的 TCP+TLS 多路复用通道。** Worker 出方向连 Server,集群侧无需开放入站端口,kubeconfig 不出集群。每个请求 / 流式会话在同一条连接上并发跑,互不阻塞 —— K8s API 代理、Helm chart blob、Pod 日志 / 终端、HTTP 与 WebSocket 反代(供 Grafana、VictoriaMetrics 等内嵌 UI 使用)、推理 SSE 流。架构记录见 [`docs/transport-v2.md`](docs/transport-v2.md)。
