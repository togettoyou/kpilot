...
//   - Skip `--model` here — the deployment generator builds it from
//     Source + SourceRef (or LocalPath / OCIURL for the matching
//     sources) at apply time so a custom row pointing at a local
//     checkpoint can reuse the same tuning flags.
...
var builtinModels = []Model{
	...
	},
	{
		...
		Image:          "vllm/vllm-openai:v0.20.2",
		Source:         ModelSourceHuggingFace,
		SourceRef:      "Qwen/Qwen3-30B-A3B-Instruct-2507",
		DefaultArgs:    `["--max-model-len","32768","--dtype","auto","--gpu-memory-utilization","0.9"]`,
		RecommendedGPU: `{"count":1,"memoryGiB":24,"model":"any"}`,
		License:        "apache-2.0",
		SortOrder:      40,
	...
	// ─── DeepSeek family ────────────────────────────────────────
	{
		...
		Image:          "vllm/vllm-openai:v0.20.2",
		Source:         ModelSourceHuggingFace,
		SourceRef:      "deepseek-ai/DeepSeek-R1",
	...
	// ─── Llama 4 family ─────────────────────────────────────────
	{
		...
		Image:          "vllm/vllm-openai:v0.20.2",
		Source:         ModelSourceHuggingFace,
		SourceRef:      "meta-llama/Llama-4-Scout-17B-16E-Instruct",
	...
	// ─── Mistral family ─────────────────────────────────────────
	{
		...
		Image:          "vllm/vllm-openai:v0.20.2",
		Source:         ModelSourceHuggingFace,
		SourceRef:      "mistralai/Mistral-Small-3.2-24B-Instruct-2506",
		DefaultArgs:    `["--max-model-len","32768","--dtype","auto","--gpu-memory-utilization","0.9"]`,
	...
	// ─── Moonshot Kimi family ───────────────────────────────────
	{
		...
		Image:          "vllm/vllm-openai:v0.20.2",
		Source:         ModelSourceHuggingFace,
		SourceRef:      "moonshotai/Kimi-K2.6",
		DefaultArgs:    `["--max-model-len","32768","--dtype","auto","--tensor-parallel-size","8","--trust-remote-code","--gpu-memory-utilization","0.9"]`,
		RecommendedGPU: `{"count":8,"memoryGiB":80,"model":"H100"}`,
	...
	for _, want := range builtinModels {
		...
		default:
			updates := map[string]any{
				...
				"image":           want.Image,
				"source":          want.Source,
				"source_ref":      want.SourceRef,
				"hf_endpoint":     want.HFEndpoint,
				"oci_url":         want.OCIURL,
				"local_path":      want.LocalPath,
