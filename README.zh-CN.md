...
| **系统监控**<ul><li>实时观测 KPilot Server 与每个 Worker 的 Go runtime —— goroutines、heap、GC pause、调度延迟、CPU%、文件描述符</li><li>WebSocket 1 Hz 推流，浏览器保留 1 小时滚动窗口</li><li>一键下载 pprof profile（heap / goroutine / CPU / allocs / block / mutex / threadcreate / trace），本机 `go tool pprof` 看火焰图</li><li>自定义业务指标 —— yamux 按集群拆分的会话 / 流数、HTTP RPS / 5xx / p99、DB 连接池、SSE 客户端数、推理反代 in-flight</li><li>零持久化、零外部依赖 —— 控制面自身的可观测性，独立于 VictoriaMetrics</li></ul> | |
