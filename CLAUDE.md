...
- **写操作保护**（基于已解析的 GVK，`_cr` URL 无法绕过）：
  - 命名空间：`kube-*` / `kpilot-*` 只读（`NAMESPACE_PROTECTED`）
  - CRD 定义：以 `*.kpilot.io` 结尾的 CRD 拒绝修改 / 删除（`CRD_PROTECTED`）
  - CR 实例：`*.kpilot.io` group 下的 CR（如 `Plugin`）拒绝写入（`CRD_PROTECTED`）
...
- 点击「启用」打开 drawer 配置 values / version / namespace，提交后下发 Helm install
- 详细启用机制见 [§ 插件系统](#插件系统)
...
- **重连补发**（`gateway.replayPendingPluginCommands`）：Worker 重连后扫描 `cluster_plugins`，对 `phase=Uninstalling && enabled=false` 重发 disable，对 `phase ∈ {Pending,Installing,Upgrading,Failed} && enabled=true` 重发 enable
