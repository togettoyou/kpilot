...
| 算力管理 (`/compute`) | GPU 资源运营 + 任务调度：HAMi vGPU、DCGM 监控、Volcano 调度、配额计费 | [docs/compute.md](docs/compute.md) |
| 模型管理 (`/models`) | 模型仓库、推理部署、调试、路由、训练任务 | [docs/models.md](docs/models.md) |
...
| P5c | 算力管理 → 任务调度（Volcano CR 浏览器子页：Queue / Job / PodGroup） | 待开始 |
