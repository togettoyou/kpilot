...
│   ├── ModelHub/            # 模型仓库（P15）：ProTable + family/runtime 过滤 + 新建 / 编辑 DrawerForm；内置预设按钮 disable + tooltip 提示；下方 Alert banner 列 P16+ roadmap
...
| P15 | 模型服务 → 模型仓库：全局 `Model` 表（store/models.go::Model + store/model.go CRUD + store/seed_models.go 11 条 2026-05 主流内置预设：Qwen3-8B/14B/32B-Instruct、Qwen3-30B-A3B-Instruct (MoE)、DeepSeek-R1、Llama-4-Scout-17B-16E-Instruct (MoE)、Mistral-Small-3.2-24B-Instruct、Phi-4、GLM-5.1、Gemma-4-31B、Kimi-K2.6，统一 pin vLLM `v0.20.2` 镜像；选型据 HF trending 2026 H1 / Artificial Analysis Intelligence Index 交叉验证）+ ModelFamily 枚举扩到 9 个（追加 `phi` / `gemma` / `kimi`）+ REST CRUD（`/api/v1/models`，handler/model.go，内置 PATCH/DELETE 返回 403 `MODEL_BUILTIN_LOCKED`，name 强 DNS-1123 label 校验便于 P16 直接用作 Deployment name）+ 前端 ModelHub（pages/ModelHub/index.tsx + ModelDrawer.tsx，ProTable + family/runtime 列过滤 + JSON args/recommended_gpu 客户端校验；FAMILY_LABELS / RUNTIME_LABELS 在 services/kpilot/model.ts 集中维护）+ 抽 `pages.common.*` 共享 i18n 键（edit/delete/copy/copied/saved/identity/runtime/tuning）+ docs/models.md 重写覆盖 schema / API / 内置预设 / FE 三段 | ✅ 完成 |
