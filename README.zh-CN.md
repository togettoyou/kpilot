...
- **集群侧零入站端口，kubeconfig 永不出集群。** Worker 出方向连 Server，通过单条 TCP+TLS 多路复用通道承载所有流量 —— K8s API 调用、Helm 安装、Pod 日志 / 终端、内嵌 UI 反代（Grafana、VictoriaMetrics）、推理 SSE 流在同一条连接上并发跑，互不阻塞。架构记录见 [`docs/transport-v2.md`](docs/transport-v2.md)。
...
**Server** 持有 UI、API 与持久化状态（集群注册表、插件元数据、账号、API 令牌、模型模板），**不保管任何 kubeconfig**。**Worker** 作为 Operator 部署在每个被纳管的集群中，通过单条 TCP+TLS 多路复用通道回连 Server 并代为执行所有 Kubernetes 操作 —— 集群侧无入站端口、无共享凭据、跨云拓扑差异对运维不可见。插件以 Helm chart 形态分发，通过集群内 CRD 协调，Helm SDK 在集群本地 RBAC 上下文中执行。
