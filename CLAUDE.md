...
| P16-B | 模型服务 → 已部署列表 + 内置 chat 调试。**协议**：`ResourceRequestStart` 加 `label_selector` 字段；worker `proxy.listFull` / `listTable` forward 到 `ListOptions.LabelSelector` / Table API `?labelSelector=` query；gateway `ResourceRequest` 同步带过去。原来 list 路径不支持 label 过滤是因为集群管理页其它列表没这个需求 —— 这里第一次用上。**已部署列表**（`GET /api/v1/models/:id/deployments`）：**零状态新表**，P16-A 生成器已经把 `app.kubernetes.io/managed-by=kpilot` + `kpilot.io/model-id=<id>` 等标签打到每个 manifest 上，**集群本身是 source of truth**。`pkg/server/api/handler/model_deploy.go::ListModelDeployments` 跨 cluster fan-out —— 遍历 `store.ListClusters()` 跳过 offline worker，每个在线 cluster 起 goroutine 8s 超时单独 ctx，`list-full apps/v1 Deployment` + 上述 selector；status 服务端预算 `Running` / `Progressing` / `Failed`；某 cluster 出错落到 `errors[]`，partial result 仍可用。用 list-full 不用 Table API 是因为需要 `spec.replicas` + `status.{ready,unavailable}Replicas` + `spec.template.spec.containers[0].image`（Table 投影都剥掉了）。**Chat 反代**（`POST /api/v1/clusters/:id/inference/:namespace/:name/*subpath` → `pkg/server/api/handler/model_chat.go::ProxyInference`）：转发到 `http://<name>.<ns>.svc.<cluster-domain>:8000/v1<subpath>`，**端口硬编码 8000**（匹配 `deploy.containerPort`，防被改造成通用集群内 HTTP 代理）；仅转发 `Content-Type + Accept` header，KPilot session cookie / Authorization 全剥掉（推理后端单租户单镜像不该泄会话）；body 上限 2 MiB / 超时 5min。**流式 = 全缓冲**（worker `HTTPResponse{Body []byte}` 设计如此）：vLLM `stream:true` 能工作但没有逐 token 体验，短回合几秒返回够用；真正 SSE pass-through 留给 P16-C（要给 `HTTPResponse` 加 `IsStream` 标志 + worker chunked 转发）。**前端**：`ModelCard` 加 `RocketOutlined` 「已部署」icon button（`onViewDeployments` 可选 prop 缺省隐藏）；`DeploymentsDrawer.tsx` 单一 Table 平铺 + status Tag + ready/total + cluster/ns/name + age + per-row Chat / Describe（跳 workloads 新 tab，不内嵌避免二级 drawer 臃肿）/ Delete（确认 modal 提示「只删 Deployment，Service/PVC/Secret 留下」）+ partial-fail Alert 列出每条 `cluster: error`；`ChatDrawer.tsx` 最小 OpenAI-compat REPL，对话状态仅在 component state（关 drawer = 新会话），每轮 send 整段 messages 上传，Enter 发送 / Shift+Enter 换行 / IME composition guard，user 右对齐 colorPrimary 头像 vs assistant 左对齐家族色头像，`whiteSpace:pre-wrap` 保留 LLM markdown 代码块换行（不上 markdown renderer 避免 XSS 风险），response 解析 `choices[0].message.content` + 兜底 `choices[0].text`，底部 token usage + 「P16-C 上线流式」一行说明 | ✅ 完成 |
