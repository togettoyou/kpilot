...
│   │   └── Volcano/         # Overview（KPI + 图表 dashboard） + Scheduler（调度策略编辑器） + VGPU（每节点 Card 列表 + Progress 仪表盘 KPI） + GPUMonitoring（NVIDIA DCGM Grafana 嵌入） + QueueQuota（单 Queue 多资源 capability/guarantee/allocated/deserved 三 tick bar） + DeviceHealth（DCGM XID/ECC/温度/显存四路 PromQL 聚合的告警列表） + GPUHour（DCGM 利用率 × 窗口积分的历史用量报表，1h/24h/7d/30d range picker） + 10 个 CR 页（Queues / Jobs / CronJobs / PodGroups / HyperNodes / JobFlows / JobTemplates / NumaTopologies / NodeShards / ColocationConfigurations）+ 对应 Form drawer（7 个类型化表单 + JobFlow/JobTemplate 走 YamlCreateDrawer / NumaTopology 只读）+ schedulerMeta + shared/Layout（NotInstalled / isResourceNotAvailable / useAutoRefresh / useStaggeredRefresh / RefreshControl / TruncatedBanner / formatAge）
...
| P13 | GPU 物理卡监控：NVIDIA DCGM Exporter 内置插件（repo chart 4.8.2，sort_order 27，DaemonSet 暴露 `:9400` Prometheus 指标）+ NVIDIA 官方 Grafana dashboard 12239（`pkg/server/dashboards/builtin/nvidia-dcgm.json`，`__inputs/__requires` 剥离 + `${DS_PROMETHEUS}` → `VictoriaMetrics` 预处理）+ 算力调度新增 `/compute/:id/gpu-monitoring` 页（`<GrafanaEmbed>` 复用，required = grafana + victoria-metrics + dcgm-exporter，i18n `pages.gpuMonitoring.*`）+ Compute sider 在 vGPU 与「调度资源」之间插一项 GPU 监控 | ✅ 完成 |
| P14a | Queue 配额深化：`pkg/server/api/handler/volcano.go::queueRow` 加 `Priority` / `Guarantee`（unwrap `spec.guarantee.resource`）/ `Deserved` 三字段；新增 `/compute/:id/queue-quota` 专题页（`pages/Compute/Volcano/QueueQuota.tsx`）—— 缩进树形 Queue Select + 主卡片 + 子 Queue 递归卡片；每资源行用纯 CSS 自绘三 tick bar（capability=track / allocated=填充 / guarantee=绿色竖线 tick / deserved=紫色竖线 tick）+ 超限 / 未达保障 Alert；复用 `useAutoRefresh` + `<RefreshControl>` | ✅ 完成 |
| P14b | 设备告警：新增 `pkg/server/api/handler/device_health.go`——`resolveVMQueryURL` 通过 plugin DB lookup 拿 VictoriaMetrics Service FQDN，4 条 PromQL 并发（XID errors / 30 分钟 ECC increase / 温度 ≥85 / FB ≥95%）走 `gw.SendHTTPRequest` worker tunnel，单 query 失败仅 log 跳过；server 预算 severity counts。前端 `/compute/:id/device-health`（`pages/Compute/Volcano/DeviceHealth.tsx`）：三 KPI 卡 + severity 多选 filter 的 ProTable，hostname 跳 `/clusters/:id/nodes`，空告警时绿色对勾 Empty。VM 未启用 → `RESOURCE_NOT_AVAILABLE` 走 `<NotInstalled>` | ✅ 完成 |
| P14c | GPU-Hour 用量报表：新增 `gpu_hour.go::GetGPUHour` 接 `?range=1h\|24h\|7d\|30d`，PromQL `avg_over_time((DCGM_FI_DEV_GPU_UTIL/100)[range:step])` × 窗口小时 = GPU-Hour 数（每 range 配套 step：1h→1m / 24h→5m / 7d/30d→15m），server 按 hours 倒序返回 + total。前端 `/compute/:id/gpu-hour`（`pages/Compute/Volcano/GPUHour.tsx`）：Radio.Group range picker + 双 Statistic 卡 + ProTable + share `<Progress>` 占比。30d range 多挂 retention warning。v1 仅按 (Hostname, gpu) 聚合，Queue/Namespace 细分留作 P14c-ext（需要 worker 周期性 Volcano 分配快照持久化） | ✅ 完成 |
