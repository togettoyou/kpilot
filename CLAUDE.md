...
│   │   ├── Logging/         # 日志页（自绘 LogsQL 搜索 + 直方图 + ns/pod picker 自动构造 stream selector）—— full-bleed 布局（外层 ResizeObserver 测 viewport - wrapperTop - footerHeight - gap，闭式公式无外层滚动条；依赖 Footer 组件的 `kpilot-footer` className 找 footer 位置）+ react-virtuoso 虚拟列表（10k 行 DOM 恒定）+ 大屏模式（Esc 退出，隐藏顶部区让 Results 占满 viewport）+ TXT/NDJSON 导出 Dropdown（纯客户端 Blob 下载）+ 共享 `<TimeRangePicker>` + 直方图默认收起
