...
│   ├── ModelHub/            # 模型仓库（P15）+ 推理部署（P16-A）：`/models/catalog`。`index.tsx` 家族分组卡片 catalog（Collapse + 搜索 + 过滤）；`ModelCard.tsx` 单卡（HF logo + 描述 + badges + Deploy 主按钮 + Edit/Duplicate/Delete）；`ModelDetailDrawer.tsx` 只读详情；`ModelDrawer.tsx` 新建/编辑/复制为自定义 三 mode；`DeployDrawer.tsx` 部署 drawer 三 tab（配置 / YAML 预览 复用 workloads YamlEditor / 部署结果）
│   ├── ModelDeployments/    # 模型实例（P16-B）：`/models/deployments`。跨模型 + 跨集群一张 ProTable（family / runtime / cluster / status 多 filter），per-row 模型调试 / Describe 跳 workloads / Delete；catalog 行被删的孤立 deployment 仍显示并打 Tag
│   ├── ModelChat/           # 模型调试（P16-B）：`/models/chat`。全屏 playground，URL 驱动 `?cluster=&ns=&name=` 选实例（grouped Select），左 Card 系统提示词 / 温度 / max_tokens 旋钮，右 Card 对话区（Row align="stretch" + 两侧 flex:1 等高），user / assistant / system 三色头像；request 用 server 下发的 `instance.model_field`（= `HuggingFaceID || deployment.name`）严格匹配 vLLM `--model` 启动值，否则 404；0 部署时整页 Result 引导跳 catalog
...
| P16-A | 模型服务 → 推理部署：`pkg/server/deploy` 生成 K8s manifests（Deployment + Service + 可选 PVC + 可选 HF Secret）→ 通过 worker tunnel apply 推到目标集群。**Model row = 模板**，同一模型可独立部署到多集群、同集群可多 instance（`{model.name}` 单实例 / `{model.name}-{instance}` 命名变体）；重复部署到同名 = SSA update。**Stateless 设计**：部署状态不进 DB，所有 manifests 打 KPilot 标签集（`app.kubernetes.io/managed-by=kpilot` + `kpilot.io/model-id` + `kpilot.io/model-family` 等），cluster 是 source of truth。`POST /api/v1/models/:id/deploy?dry_run=true` 给前端预览 YAML。GPU plumbing 二选一：`nvidia.com/gpu`（默认）或 `volcano.sh/vgpu-number`（需 volcano-vgpu-device-plugin）。HF token 落 Secret + envFrom 注入容器。PVC 默认开（heuristic 按模型名预估大小：0.6B→5G / 32B→100G / R1→1.4T），避免容器重启重新下载权重。dshm tmpfs 2 GiB 兜底 NCCL。**容器 resources 三层叠加**：① cpu/memory request + limit 可不同（burst headroom，K8s quantity strings 经 `resource.ParseQuantity` 校验）② GPU count 走 `limits` only（K8s 在 admission 镜像到 requests；device-plugin extended resource 强制 requests==limits）③ Volcano vGPU 子资源（`vgpu-memory` MiB + `vgpu-cores` 0-100%）limit-only，**仅当 gpu_type=volcano 时下发**。Container 还兜底 dshm tmpfs 2 GiB 给 NCCL，readiness probe 在 /health 上 5min failureThreshold 等 HF 冷下载。**前端 `ModelHub/DeployDrawer.tsx`** 三 tab 设计：「配置」（cluster Select / namespace / instance / replicas / GPU 数 / GPU 类型 radio / CPU+内存 request+limit 2×2 / Volcano vGPU sub-resources 条件渲染 / extra_args 每行一个 / HF Token 在表单底部 + autoComplete=new-password 防浏览器误填 / PVC 开关 + 大小）+「YAML 预览」（切 tab 自动 dry_run，复用 workloads `YamlEditor` 拿 CodeMirror + YAML mode + 主题色，表单 onValuesChange 失效缓存）+「部署结果」（仅首次提交后出现，per-doc Alert + Table，错误列 whiteSpace=pre-wrap 完整显示 K8s 错误信息）。Deploy 按钮 `disabled={!isFormReady}`（4 个 `Form.useWatch` 监听必填字段），未填完整不可点 + 预览 tab 显示提示而非空编辑器。`POST /api/v1/models/:id/deploy?dry_run=true` 走预览路径。"已部署列表" UX 暂不做（要求加 LabelSelector 到 proto，留给 P16-B 一起）。**T4 实测验证**：腾讯云 Tesla T4 单卡节点（k3s + nvidia-container-toolkit + drop-in `/var/lib/rancher/k3s/agent/etc/containerd/config-v3.toml.d/50-nvidia-default.toml` 把 `default_runtime_name=nvidia`）跑通 Qwen3-0.6B 全链路 —— UI 一键部署 → Pod Running → `/v1/chat/completions` 实际返回带 thinking 模式的 Qwen3 输出，P16-A 推理路径端到端验证 | ✅ 完成 |
| P16-B | 模型服务 → 部署实例 + 内置 chat 调试。**协议**：`ResourceRequestStart` 加 `label_selector` 字段；worker `proxy.listFull` / `listTable` forward 到 `ListOptions.LabelSelector` / Table API `?labelSelector=` query；gateway `ResourceRequest` 同步带过去。原来 list 路径不支持 label 过滤是因为集群管理页其它列表没这个需求 —— 这里第一次用上。**菜单结构**：`/models` 改成三个对等子菜单页（不再是单页 + 抽屉链）—— `/models/catalog`（模型仓库）/ `/models/deployments`（部署实例）/ `/models/chat`（Chat 调试），路由在 `web/config/routes.ts` 父菜单加 `routes:` 数组，对齐集群管理 / 算力调度的菜单形式。**部署实例端点**（`GET /api/v1/models/deployments[?model_id=N]`）：**零状态新表**，P16-A 生成器已经把 `app.kubernetes.io/managed-by=kpilot` + `app.kubernetes.io/component=inference` + `kpilot.io/model-id=<id>` 等标签打到每个 manifest 上，**集群本身是 source of truth**。`pkg/server/api/handler/model_deploy.go::ListAllDeployments` 一个端点两种模式（无参 = 全集 / `model_id=N` = 单模型过滤）；启动时一次 `store.ListModels` 装 `modelsByID` map 避免 N+1 DB 查询；跨 cluster fan-out —— 遍历 `store.ListClusters()` 跳过 offline worker，每个在线 cluster 起 goroutine 8s 超时单独 ctx，`list-full apps/v1 Deployment` + 上述 selector；每行 enrich `ModelDisplayName / Family / Runtime`（catalog 行被删的孤立 deployment 保留显示 + 前端打「孤立」Tag）；status 服务端预算 `Running` / `Progressing` / `Failed`；某 cluster 出错落到 `errors[]`，partial result 仍可用。用 list-full 不用 Table API 是因为需要 `spec.replicas` + `status.{ready,unavailable}Replicas` + `spec.template.spec.containers[0].image`（Table 投影都剥掉了）。**Chat 反代**（`POST /api/v1/clusters/:id/inference/:namespace/:name/*subpath` → `pkg/server/api/handler/model_chat.go::ProxyInference`）：转发到 `http://<name>.<ns>.svc.<cluster-domain>:8000/v1<subpath>`，**端口硬编码 8000**（匹配 `deploy.containerPort`，防被改造成通用集群内 HTTP 代理）；仅转发 `Content-Type + Accept` header，KPilot session cookie / Authorization 全剥掉（推理后端单租户单镜像不该泄会话）；body 上限 2 MiB / 超时 5min。**流式 = 全缓冲**（worker `HTTPResponse{Body []byte}` 设计如此）：vLLM `stream:true` 能工作但没有逐 token 体验，短回合几秒返回够用；真正 SSE pass-through 留给 P16-C（要给 `HTTPResponse` 加 `IsStream` 标志 + worker chunked 转发）。**前端 / 部署实例页** `pages/ModelDeployments/index.tsx`：单一 ProTable 平铺 + column filters (Cluster/Runtime/Status) + family 快筛 `<Tag.CheckableTag>` 行（仅显示数据里实际出现的家族）+ Model 列 Avatar + 孤立 Tag + per-row Chat（`history.push('/models/chat?cluster=...&ns=...&name=...')`）/ Describe（跳 workloads 新 tab）/ Delete（确认 modal 提示「只删 Deployment，Service/PVC/Secret 留下」）+ partial-fail Alert 列出每条 `cluster: error`。**前端 / Chat playground** `pages/ModelChat/index.tsx`：双列 Card 布局（左：实例 Select grouped by model + 系统提示词 + temperature/max_tokens 旋钮 + 流式说明 Alert；右：对话区），URL 驱动 `?cluster=&ns=&name=` 选择（`history.push` 可分享 / 收藏），切实例自动清空 history，0 部署整页 Result 引导跳 catalog；对话气泡 user 右对齐 colorPrimary / assistant 左对齐家族色 / system 灰，`whiteSpace:pre-wrap` 保留 markdown 代码块换行（不上 markdown renderer 避免 XSS）；Enter 发送 / Shift+Enter 换行 / IME composition guard；response 解析 `choices[0].message.content` + 兜底 `choices[0].text`；model 字段用 `deployment.name`（DNS-1123-safe 比 hugging_face_id 在日志里更可识别）；底部 token usage。**cleanup**：原 `ModelHub/DeploymentsDrawer.tsx` + `ChatDrawer.tsx` + ModelCard 上的 `RocketOutlined` 已部署按钮全部删除（per-model drawer 强行切片跨模型视图，菜单形式更对劲）。**UX 收尾**：(1) **vLLM 404 修复** —— 第一版前端按 `deployment.name` 发 `model` 字段，vLLM 启动用 `--model <HuggingFaceID>` 严格匹配，所有 chat 请求都 404 `does not exist`。改成 server 端 `ModelInstance.ModelField = HuggingFaceID || deployment.name` 下发，前端直接用，不在客户端拼装。(2) **菜单 + 页标题改名**：「部署实例」→「模型实例」（en: Instances）、「Chat 调试」→「模型调试」（en: Playground），避免 v1 的 chat-debug Chinglish 感。(3) **三页统一 `breadcrumbRender={false}`** + 删 catalog 的「即将推出」roadmap Alert 与「模型部署 / 调试 / 路由在后续版本落地」subtitle —— 这些功能现在都在了，banner 反而误导。(4) **chat 页左右 Card 等高** —— Row `align="stretch"` + 固定 `height: calc(100vh - 200px)` + 两侧 Card `flex:1`；第一版左侧 Card grow with content / 右侧 fixed viewport height 出现错位 | ✅ 完成 |
| P16-C | 模型服务 → OpenAI 兼容反代 endpoint + 真正的 SSE 流式底座。**传输层**：`HTTPRequestStart` 加 `bool stream_response = 4`（向后兼容,老 worker 忽略默认 false 走 buffered）。worker `pkg/worker/proxy/http.go::handleStreaming` 在 stream=true 时不 `io.ReadAll`,改 32 KiB 缓冲循环 Read → `SendHTTPResponseChunk` 边读边发。worker `tunnel/chunked.go` 拆出 `sendHTTPResponseStart / Chunk / End` 三个 incremental API,通过 `Client.SendHTTPResponseStart/Chunk/End` 暴露给 proxy。**Gateway**：`pkg/server/gateway/http_stream.go` 新增 `HTTPStream{Status, Headers, Error, Chunks <-chan []byte, EndErr <-chan error, Close()}` + `SendHTTPRequestStream(ctx, clusterID, req)`,session 在 `g.httpStreams[requestID]` 注册,recv loop 在分发 `HttpRespStart / BodyChunk / BodyEnd` 时**优先 check streaming session 再 fall back 到原 buffered rxAccumulator**。chunks channel buffer 32(2 MiB worst case),`push()` 满时阻塞本 worker recv 循环(其它 worker 不受影响)—— handler MUST defer `stream.Close()`。worker 断开时 `closeAllHTTPStreams` 兜底强制关掉所有 active session。`HTTPRequest` 加 `StreamResponse bool` 字段透传;原 `SendHTTPRequest` 路径零影响(Grafana / VM / VL 保持 buffered)。**鉴权**：新表 `pkg/server/store/api_key.go::APIKey{ID, Name, TokenHash(sha256), TokenPrefix, ClusterID, Namespace, DeployName, LastUsedAt(throttled 1/min), RevokedAt, ...}`,Token 格式 `kp-sk-<24 byte crypto/rand base64url>` 一次性明文展示,sha256 入库。中间件 `middleware/bearer_api_key.go::BearerAPIKey(clusterParam, nsParam, deployParam)` 提取 `Authorization: Bearer ...`,sha256 → store.GetAPIKeyByHash → 检查 RevokedAt + URL path scope 精确匹配 → 异步 `TouchAPIKeyLastUsed`(throttled via WHERE-clause guard)。**REST**：`POST/GET /api/v1/api-keys` + `POST /:id/revoke` + `DELETE /:id`(操作员 JWT cookie 鉴权);**对外 endpoint** `Any /api/v1/clusters/:id/proxy/inference/:namespace/:name/*subpath` 走 `BearerAPIKey` 中间件 → `handler/inference_proxy.go::ProxyInferenceOpenAI` → `SendHTTPRequestStream` → 共享 `writeStreamingResponse` 把 Status+Headers 透传 + 每 chunk `Flush()` + `X-Accel-Buffering: no` 头告诉 nginx-ingress 不 buffer SSE。worker dispatch 失败(`stream.Error != ""`)走 502 + `PROXY_UPSTREAM_ERROR`;upstream 中途截断只 log 不 panic(HTTP 200 已发,SDK 自己感知 SSE 截断)。**P16-B 同步切流式**：`handler/model_chat.go::ProxyInference` 改用 `SendHTTPRequestStream` 共享 `writeStreamingResponse`,鉴权保持 cookie URL 保持 `/inference/:ns/:name/*subpath`(无 `/proxy/` 段)。**前端**：`services/kpilot/model.ts::streamChatCompletions(target, body, handler, signal)` 用 fetch + ReadableStream + TextDecoder(umi `request` 全缓冲不可用),按 `\n\n` 或 `\r\n\r\n` 分 SSE 事件,`data:` 行 JSON.parse `choices[0].delta.content` 触发 `onDelta`,`[DONE]` 或 `finish_reason` 触发 `onDone`,`usage` 触发 `onUsage`。`pages/ModelChat/index.tsx` 发送时**预分配 assistant 气泡固定 React key**(否则反复 remount 丢帧),onDelta 走函数式 `setHistory((prev) => ...)` append last 消息;Stop 按钮在 sending=true 时替换 Send,调 AbortController.abort();移除过时的 streamNote Alert。AbortError 不算 error。错误码 4 个 `API_KEY_NOT_FOUND/INVALID/MISSING/SCOPE_MISMATCH` 中英双套翻译。**已知边界**：(1) chunks channel 满阻塞 recv 循环—defer Close 必须;(2) APIKey 无配额/token 计数(P16-D+);(3) APIKey 前端 UI 延后到 P16-D,P16-C v1 操作员用 curl + JWT cookie 管理 | ✅ 完成 |
| P16-D | 模型服务 → APIKey 前端管理页 + token usage 计数 | 待开始 |
