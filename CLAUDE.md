...
| `/system` | 系统监控 | KPilot 自身（server + 每个 worker）的运行时观测；landing = 节点表，进入 `/system/:node` 是详情页（WS 推 1Hz 实时图 + pprof 下载）。完全不依赖 VM/Grafana，所有数据来自 `pkg/diag` 内置采集。**放在 sider 最右**——定位是诊断工具，不是日常主流程入口 |
...
| P19 | **第 5 平台「系统监控」(`/system`):KPilot 自身 server + worker 进程的运行时观测,完全脱离 VM/Grafana**。<br/>**架构**:三层包,通用底座 + 业务 collector 分离,future-proof 可拆库。`pkg/diag/`(零业务依赖,纯 stdlib,~370 LOC):`Diag.New(kind, name, version)` + `Register(Collector)` + `Snapshot()`,`Mount(mux, prefix)` 挂 `/info` + `/snapshot` + `/pprof/*`(stdlib `net/http/pprof`,显式注册 6 个 named profile + Cmdline/Profile/Symbol/Trace,不用 `pprof.Index` 因为它硬编码 `/debug/pprof/` 前缀);runtime/metrics 投影 50+ key(heap 各段 + GC pause p50/p90/p99 max + sched latency 分位 + CPU 各类 + mutex wait + alloc rate + live objects + RSS Linux only),`samples []metrics.Sample` 切片复用避免每次 5 KB 分配,histogram 用 live/prev delta tracker(5s gap 重置 baseline)取代 lifetime smoothing。`pkg/worker/diag/`:TunnelCollector(connected/uptime/reconnect_total/streams_open,全 atomic)+ ProxyCollector(5 个 handler inflight atomic.Int32)+ RouterCollector(InClusterRouter hit/miss + hit_rate);`Serve(ctx, *diag.Diag)` bind 127.0.0.1:0 一个 `http.ServeMux`,返回端口号给 `tunnelClient.SetDiagPort`。`pkg/server/diag/`:YamuxCollector(GatewayServer.DiagSnapshot:sessions/streams_open/streams_by_cluster)+ DBCollector(sql.DBStats)+ HTTPCollector(关键)+ InferenceCollector(全局 atomic 计数 InferenceInflight/Total/SSEClients,handler 直接 `diag.InferenceInflight.Add(1); defer Add(-1)` 埋点)。<br/>**HTTPCollector 关键性能设计**:双 buffer live/prev 模式 + janitor 1s 旋转(`prev = live.Swap(0)`):hot path 每请求 5–7 个 atomic 操作(in_flight ± / total ± / liveReqs + / liveLat[bucket] +),**零 CAS 重试 / 零 mutex**;p50/p90/p99 算在 24 桶 power-of-2 latency histogram 上(1ms → 16s),计算完全在 reader 路径外(rotate goroutine 独占 swap)。Gin middleware 第一个 `r.Use()` 上,捕获所有请求。<br/>**Proto v2 扩展**:`RegisterRequest` 加 `uint32 diag_port = 4`(向后兼容),worker `tunnel.Client.SetDiagPort(port)` 在 `Run()` 之前调,handshake 时上报;`gateway.ConnectedWorker` 加 `DiagPort uint32` 字段记录。<br/>**Server REST + WS hub**(`pkg/server/api/handler/system.go`):5 个端点 `/api/v1/system/nodes`(列 server + worker)、`/snapshots`(并发 fan-fetch + 800ms ctx/节点 + partial-fail)、`/:node/snapshot`(单节点)、`/:node/pprof/:kind`(下载 `.pb.gz`,`profile`/`trace` 强制 `?confirm=true` 否则 403 `PPROF_CONFIRMATION_REQUIRED`)、`/:node/stream`(WS)。**WS fan-out hub 关键并发设计**:`systemHub` keyed by nodeID,每个 `nodeStream` 维护 subs map + stop chan + `atomic.Pointer[[]byte] last`(给新订阅者立即推最近一次);第一个 subscriber 进入 → `go runNode(ns)` 启动 1Hz ticker,最后一个出 → cleanup(在 `h.mu` 下 re-check subs 空 → `delete(h.nodes, ...)` + `stopOnce.Do(close stop)`)。**同节点 N 个 browser 共用 1 次/秒拉流,不会因为多 tab 把 worker 拉 N 次**;snapshot fan-out 用 per-WS write goroutine(每 1s 一波,10s `writeWait` 上限),wsConn.WriteMessage 带 writeMu 跟 ping/pong 并发安全。**Snapshot 透明反代**:server 本地 / worker 远端两条路径统一走 `proxyDiagGET` —— server 直接 loopback `http.Get http://127.0.0.1:<server-diag-port>/debug/snapshot`(`cmd/server/main.go` 起独立 `serveDiag` listener),worker 走 `gw.SendHTTPRequest(clusterID, "http://127.0.0.1:<DiagPort>/...")`;同一份 HTTPProxy 反代代码,零新增传输路径。<br/>**前端 `/system`**:landing `pages/System/index.tsx` = ProTable + 4s `setInterval` polling `batchSystemSnapshots()`,显示节点类型 / 状态(在线 / 离线 / 无指标)/ uptime / goroutines / heap / GC p99 / RSS / 业务 KPI(server:sessions/streams/RPS;worker:tunnel↑↓ + streams + inflight)/ 查看按钮。详情页 `pages/System/Detail/index.tsx`:**ringBuffer 用 useRef 存 3600 点(1Hz × 1h)**,React state 只放最新 snapshot + tick counter,防止整页 re-render;WebSocket onmessage → push ring + `setTick(t+1)`,charts useMemo([tick]) 重算 series;**8 KPI 卡 + 7 tab**(概览/内存/调度/网络[server]/HTTP[server]/数据库[server]/集群代理[worker]/pprof),antd Tabs `destroyOnHidden` 避开 G2 hidden-pane 闪烁;CPU% 客户端从 ring buffer 末两点 delta 算(busy = user+gc+scavenge over total)。**SystemChart** = `@ant-design/plots` Line 2.x 薄包(autoFit + axis x/y v2 API + scale domainMin:0),用 `React.lazy` 把 G2 ~250 KB bundle 切到详情页才下载。pprof tab:6 个低开销按钮(heap/goroutine/allocs/block/mutex/threadcreate)直接 `window.open` 下载 .pb.gz;CPU profile / trace 两个高开销 `danger` 按钮,Modal 二次确认才带 `confirm=true` 打开。WS 自动重连(1s → 5s 指数退避),断开期间显示 warning Alert。<br/>**特性**:零持久化,完全 in-memory + browser ring buffer;后端**无外部依赖**(纯 stdlib + 已有的 gorilla/websocket);性能开销 = 每进程 +1 sleeping goroutine 空载(开页面 +1Hz `metrics.Read()` ~10µs),hot path 零 mutex。`-race` 跑通所有 `pkg/diag/` + `pkg/server/diag/` 测试。<br/>**T4 实测**:本地 server 起 → `/api/v1/system/server/snapshot` 返回完整 JSON(50+ runtime + 4 个 custom collector 数据);WS stream 3 帧 1.5 KB/s;`heap.pb.gz` 7136 bytes 合法 gzip;`/profile?seconds=2` 无 confirm = 403 `PPROF_CONFIRMATION_REQUIRED`,带 confirm = 200 + .pb.gz。 | ✅ 完成 |
