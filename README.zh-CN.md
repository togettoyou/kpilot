...
- **Chunked 分片 + gzip 压缩 + 公平队列。** 大 payload（chart .tgz、describe 输出、log tail）按 ≤64 KiB 切片，gRPC stream 级 gzip 压缩（JSON 类响应 5–8× 缩减）；每条流单一 sender goroutine 先 drain Heartbeat fast lane，再对 per-request 子队列 round-robin 调度 —— 一个 20 MiB 日志响应不会 head-of-line block 同时发的 `/workloads/nodes` 小请求。worker 离线检测独立走 gRPC HTTP/2 keepalive PING，与应用心跳解耦。
...
chart 自动生成共享 token、自动把 Worker 指向集群内的 gRPC Service，Server 启动时自动注册一个名为 `local` 的集群行。无需进 UI 手动创建条目，数秒后该集群即 Online。
...
helm install kpilot-worker oci://ghcr.io/togettoyou/charts/kpilot \
  ...
  --set worker.serverAddr='<Server gRPC 外部地址>:9090' \
