...
FROM = sys.argv[4]
TO = sys.argv[5]
LIMIT = sys.argv[6] if len(sys.argv) > 6 else '10000'
DELAY_MS = int(sys.argv[7]) if len(sys.argv) > 7 else 300
...
threads = [threading.Thread(target=req, args=('logs',
    f'{HOST}/api/v1/clusters/{CID}/logs/search?query=*&from={FROM}&to={TO}&limit={LIMIT}'))]
threads[0].start()
