...
多集群是默认能力 —— 一个 KPilot Server 纳管多个集群,由集群内 Worker 主动通过单条长连接 TCP 多路复用通道回连(生产部署在 ingress 层做 TLS 终止)。集群侧无需开放入站端口、无需共享 kubeconfig、无需为不同云做差异化适配。

## 为什么是 KPilot

- **集群侧零入站端口,kubeconfig 永不出集群。** Worker 出方向连 Server,通过单条 TCP 多路复用通道承载所有流量(生产部署在集群 ingress 层做 TLS 终止)—— K8s API 调用、Helm 安装、Pod 日志 / 终端、内嵌 UI 反代(Grafana、VictoriaMetrics)、推理 SSE 流在同一条连接上并发跑,互不阻塞。架构记录见 [`docs/transport-v2.md`](docs/transport-v2.md)。

- **GPU + Volcano 一体化平台。** Volcano gang scheduling 覆盖 10 个 CR 类型，7 个类型化创建表单 + 可视化调度策略编辑器；vGPU device-plugin 切片（slot / 显存 / SM 算力）按物理卡解析，列出当前持有每个切片的 Pod；DCGM 驱动的 GPU-Hour 用量报告（1h / 24h / 7d / 30d 窗口）+ XID / ECC / 温度 / 显存压力四类告警 —— 一个控制台同时回答"谁调度的、谁在用、硬件健不健康"。

- **应用内模型服务，catalog 到 API 全链路。** 精选开源权重 LLM 一键部署（Qwen3、DeepSeek-R1、Llama-4、Mistral、Phi-4、GLM-5.1、Gemma-4、Kimi-K2.6 —— 默认 vLLM）到任意纳管集群，可选 vGPU 资源精细切分；浏览器内 chat playground 调试每个实例；按部署签发受限范围的 OpenAI 兼容 Bearer 令牌给业务方使用 —— 无需独立运维一套模型服务栈。

- **Plugin-first 平台。** KPilot 自身的观测栈（VictoriaMetrics、VictoriaLogs、Grafana、DCGM Exporter、Metrics Server、kube-state-metrics）走的就是操作员用来安装任意客户 chart 的那条**同一条**内置 Helm 注册表流水线。按集群启用 / 禁用 / 升级，安装日志实时回流 UI；按集群覆盖 values；自定义 chart 同样接入。
...
**Server** 持有 UI、API 与持久化状态(集群注册表、插件元数据、账号、API 令牌、模型模板),**不保管任何 kubeconfig**。**Worker** 作为 Operator 部署在每个被纳管的集群中,通过单条长连接 TCP 多路复用通道回连 Server(生产部署在集群 ingress 层做 TLS 终止)并代为执行所有 Kubernetes 操作 —— 集群侧无入站端口、无共享凭据、跨云拓扑差异对运维不可见。插件以 Helm chart 形态分发,通过集群内 CRD 协调,Helm SDK 在集群本地 RBAC 上下文中执行。
...
| | |
|---|---|
| **集群管理**<ul><li>单 Token 一次性纳管,无需共享 kubeconfig</li><li>节点与工作负载实时浏览,覆盖原生与自定义资源</li><li>浏览器直接调取 Pod 日志、终端、按容器查看 CPU / 内存即时指标</li><li>内置 YAML 编辑器,对任意资源执行 apply / describe / delete</li><li>自绘监控页(Cluster / Node / Pod 三 tab + 分组 LazySection)与日志页(LogsQL 搜索 + 实时尾随 + 直方图点击 zoom + 行展开 + 关键词高亮),不嵌 Grafana</li></ul> | **算力调度**<ul><li>基于 Volcano 的 gang scheduling,覆盖 Queue / Job / CronJob / PodGroup / HyperNode</li><li>借助 volcano-vgpu-device-plugin 实现 GPU 精细切片(按显存槽位、显存量、SM 算力)</li><li>多资源队列配额可视化(capability / guarantee / allocated / deserved 四维拆解)</li><li>调度策略可视化编辑器 —— actions、tier、plugin 参数全字段提示</li></ul> |
| **GPU 可观测性**<ul><li>物理卡级面板:利用率、温度、功耗、显存、SM 频率、Tensor 活动 —— 节点/GPU 多选筛选 + chart 阈值线(温度 80/90°C)+ 每图全屏放大</li><li>「需要关注的 GPU」客户端聚合 idle / 过热 / 显存告急 三组,健康集群自动隐藏</li><li>基于 DCGM 的 GPU-Hour 用量报告,支持 1h / 24h / 7d / 30d 窗口</li><li>DCGM XID、ECC、温度、显存压力四类告警一站汇聚</li><li>vGPU 视图按物理卡列出当前持有切片的所有 Pod</li></ul> | **插件管理**<ul><li>内置 Helm 注册表 —— Volcano、DCGM Exporter、VictoriaMetrics、VictoriaLogs、Grafana、Metrics Server、kube-state-metrics 开箱即用</li><li>按集群启用 / 禁用 / 升级,安装日志实时流回 UI</li><li>支持自定义 chart,按集群覆盖 values</li><li>KPilot 自身的可观测性栈即由这条插件流水线启动</li></ul> |
| **模型服务**<ul><li>精选目录:Qwen3-0.6B/8B/14B/32B-Instruct、Qwen3-30B-A3B (MoE)、DeepSeek-R1、Llama-4-Scout-17B-16E (MoE)、Mistral-Small-3.2-24B、Phi-4、GLM-5.1、Gemma-4-31B、Kimi-K2.6 —— 默认 vLLM</li><li>一键部署到任意纳管集群,支持 `nvidia.com/gpu` 或 Volcano vGPU 资源精细切分,HF token 通过 Secret + envFrom 注入,PVC 按模型大小启发式预估</li><li>跨集群 ProTable 列出所有运行实例,逐行可调试 / Describe / 级联删除(Deployment + Service + PVC + Secret 一键清理)</li><li>浏览器内 chat playground 做冒烟测试:每轮 tok/s 速率展示、`<think>…</think>` 推理过程自动折叠、Markdown 渲染代码块 / 列表 / 表格</li></ul> | **OpenAI 兼容网关**<ul><li>逐部署签发 Bearer 令牌(`kp-sk-…`,sha256 哈希入库,一次性明文展示)</li><li>端到端 SSE 流式 —— vLLM `stream: true` 的 token 实时到达 SDK;浏览器 Stop 经 yamux FIN 链路秒级杀掉 upstream</li><li>**逐 Key 用量计量**(prompt / completion / 调用数)实时从 upstream `usage` 字段解析;操作员可一键重置计量窗口</li><li>令牌创建时二级 scope picker(集群 → 部署)防止 scope 错配</li><li>软撤销(保留审计行)+ 硬删除两种 row action</li></ul> |
