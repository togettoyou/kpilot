...
│   │   └── Volcano/         # Overview（KPI + 图表 dashboard） + Scheduler（调度策略编辑器） + VGPU（每节点 Card 列表 + Progress 仪表盘 KPI） + GPUMonitoring（NVIDIA DCGM Grafana 嵌入） + QueueQuota（单 Queue 多资源 capability/guarantee/allocated/deserved 三 tick bar） + 10 个 CR 页（Queues / Jobs / CronJobs / PodGroups / HyperNodes / JobFlows / JobTemplates / NumaTopologies / NodeShards / ColocationConfigurations）+ 对应 Form drawer（7 个类型化表单 + JobFlow/JobTemplate 走 YamlCreateDrawer / NumaTopology 只读）+ schedulerMeta + shared/Layout（NotInstalled / isResourceNotAvailable / useAutoRefresh / useStaggeredRefresh / RefreshControl / TruncatedBanner / formatAge）
...
| P14b | 设备健康告警：DCGM XID / ECC 异常聚合 + vGPU `health=false` 卡片汇总通知中心 | 待规划 |
| P14c | GPU-Hour 计费报表：按 queue / namespace 聚合的历史 GPU-Hour 用量 | 待规划 |
