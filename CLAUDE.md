...
│   │   └── Volcano/         # Overview（KPI + 图表 dashboard） + Scheduler（调度策略编辑器） + VGPU（每节点 Card 列表 + Progress 仪表盘 KPI） + GPUMonitoring（NVIDIA DCGM Grafana 嵌入） + 10 个 CR 页（Queues / Jobs / CronJobs / PodGroups / HyperNodes / JobFlows / JobTemplates / NumaTopologies / NodeShards / ColocationConfigurations）+ 对应 Form drawer（7 个类型化表单 + JobFlow/JobTemplate 走 YamlCreateDrawer / NumaTopology 只读）+ schedulerMeta + shared/Layout（NotInstalled / isResourceNotAvailable / useAutoRefresh / useStaggeredRefresh / RefreshControl / TruncatedBanner / formatAge）
...
| P14 | 资源治理：Volcano queue 配额视图 + 设备健康告警 + GPU-Hour 计费报表 | 待规划 |
